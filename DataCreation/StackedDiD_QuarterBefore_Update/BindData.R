...
dir <- "~/Dropbox/CreditCards/Data/DiD_QuarterBeforeControl_Update/"
...
# create df of employees employed the entire period 
for (f in files){
  # update count 
  count = count + 1
  # read in file 
...
for (f in files){
  ...
  dta_agg[, FullEmployPre := ifelse(NumMonths[Post == 0][1] == 3, 1, 0), .(ARM_ID_NBR, TICKER)]
  dta_agg[, FullEmployPost := ifelse(NumMonths[Post == 1][1] == 3, 1, 0), .(ARM_ID_NBR, TICKER)]
