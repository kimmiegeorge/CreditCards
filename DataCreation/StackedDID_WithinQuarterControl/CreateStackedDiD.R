# Create Stacked DiD Data - Within Quarter Control 
...
output_dir <- "~/Dropbox/CreditCards/Data/DiD_TwoMonthsBefore/Event_files/"
...
for (i in seq_len(nrow(treat_obs))){
  ...
  yms <- unique(ea_month_data[TICKER == tic & EA_ID == CurrentEA]$YearMonth)
  post_ym <- yms[length(yms)]
  
  # treat obs 
  treat <- ea_month_data[TICKER == tic & EA_ID == CurrentEA]
  treat[, Treat := 1]
  treat[, Post := ifelse(YearMonth == post_ym, 1, 0)]
  ...
  control[, Treat := 0]
  control[, Post := ifelse(YearMonth == post_ym, 1, 0)]
...
for (e in events){
  ...
  filename <- paste0(output_dir, file)
